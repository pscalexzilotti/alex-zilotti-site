    ...
    <section style={{
      ...
      <motion.div {...up(0.9)} style={{ display: 'flex', alignItems: 'center', gap: 32, flexWrap: 'wrap' }}>
        ...
        <Link href="/" className="back-link" style={{
          ...
          color: 'var(--stone)', display: 'flex', alignItems: 'center', gap: 10, transition: 'color 0.3s',
        }}>
          ← Página Inicial
        </Link>
      </motion.div>
...
/* ─── Cinematic strip com foto real ─── */
export function CinematicStrip() {
  return (
    <div style={{ position: 'relative', height: 360, overflow: 'hidden' }}>
      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage: 'url(/juridico.jpg)',
...
/* ─── 6 serviços ─── */
const servicos = [
  { num: '01', title: 'Análise Crítica de Laudos', desc: 'Leitura especializada de laudos periciais psicológicos, identificando inconsistências metodológicas, lacunas técnicas e pontos de questionamento estratégico para a defesa ou acusação.' },
  ...
  { num: '04', title: 'Acompanhamento de Perícias', desc: 'Presença técnica durante realização de perícias psicológicas, assegurando que o processo avaliativo siga protocolos adequados e identificando eventuais irregularidades procedimentais.' },
  { num: '05', title: 'Estratégia Processual Psi', desc: 'Consultoria estratégica sobre como a dimensão psicológica do caso pode ser instrumentalizada processualmente — quando e como acionar argumentos psicológicos com maior eficiência.' },
  { num: '06', title: 'Apoio em Audiências', desc: 'Suporte técnico especializado durante audiências que envolvam depoimentos, avaliações ou argumentos psicológicos, com orientação em tempo real para o advogado.' },
]
  ...
  return (
    <section style={{ background: 'var(--ink)', padding: '128px 64px' }}>
      ...
      <div className="svc-jur-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, background: 'rgba(154,128,96,0.07)' }}>
        {servicos.map((s, i) => (
          <RevealWrapper key={s.num} delay={i * 0.07}><SvcCard {...s} /></RevealWrapper>
        ))}
      </div>
      <style>{`
