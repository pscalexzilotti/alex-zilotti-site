  ...
  return (
    <section style={{
      ...
      <motion.p {...up(0.7)} style={{
        fontSize: '0.92rem', color: 'var(--stone)', lineHeight: 1.9, maxWidth: 540, marginBottom: 56,
      }}>
      ...
      <motion.div {...up(0.9)} style={{ display: 'flex', alignItems: 'center', gap: 32, flexWrap: 'wrap' }}>
        ...
        <Link href="/" className="back-link-cli" style={{
    ...
    <section style={{ background: 'var(--charcoal)', padding: '128px 64px' }}>
      ...
      <RevealWrapper delay={0.15}>
        <p style={{ fontSize: '0.88rem', color: 'var(--stone)', lineHeight: 1.9, maxWidth: 600, marginBottom: 64 }}>
          A Terapia Cognitivo-Comportamental é reconhecida cientificamente como uma das abordagens
          mais eficazes para o tratamento de uma ampla gama de condições. Atendimento especializado
          para profissionais de alta performance e para quem busca mudanças reais e mensuráveis.
        </p>
      </RevealWrapper>
      <div className="esp-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, background: 'rgba(154,128,96,0.07)' }}>
        {especialidades.map((e, i) => (
          <RevealWrapper key={e} delay={i * 0.04}><EspCard title={e} /></RevealWrapper>
        ))}
      </div>
      <style>{`
...
function EspCard({ title }: { title: string }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
      ...
      borderTop: '1px solid rgba(154,128,96,0.15)',
...
function ImersaoCard({ num, title, desc }: { num: string; title: string; desc: string }) {
  const [hovered, setHovered] = useState(false)
  return (
    <div className="im-card" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
      padding: '32px 36px',
      background: hovered ? 'var(--graphite)' : 'var(--charcoal)',
      transition: 'background 0.4s var(--ease)', display: 'flex', gap: 24, alignItems: 'flex-start',
    }}>
      ...
      <div className="workshop-body" style={{ padding: '80px 64px 128px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80 }}>
        <RevealWrapper>
          <p style={{ fontSize: '0.9rem', color: 'var(--stone)', lineHeight: 2, marginBottom: 24 }}>
            O Workshop Rotas de Alta Performance Emocional é uma experiência clínica intensiva
          ...
          <p style={{ fontSize: '0.9rem', color: 'var(--stone)', lineHeight: 2 }}>
            ...
            em um processo personalizado — uma recalibração profunda dos padrões mentais e emocionais
            que determinam seus resultados.
          </p>
        </RevealWrapper>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, background: 'rgba(154,128,96,0.07)' }}>
          {[
            ...
            <RevealWrapper key={c.num} delay={i * 0.12}><ImersaoCard {...c} /></RevealWrapper>
          ))}
        </div>
      </div>
      ...
      <div className="abordagem-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, maxWidth: 1200, margin: '0 auto', alignItems: 'center' }}>
        <RevealWrapper>
          <div style={{ position: 'relative', overflow: 'hidden', aspectRatio: '7/5' }}>
            <div style={{
              position: 'absolute', inset: 0,
              backgroundImage: 'url(/clinica.jpg)',
